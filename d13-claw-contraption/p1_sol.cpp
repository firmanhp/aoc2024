...
constexpr int COST_A = 3;
constexpr int COST_B = 1;
...
int find_tokens(const TestCase& testcase) {
  Vector2D<ll> mat(2, 2);
  ...
  std::vector<ll> res(2);
  res[0] = testcase.target.first;
  res[1] = testcase.target.second;
  ...
  const int token_a = COST_A * (adj_res[0] / det);
  const int token_b = (adj_res[1] / det);
  // std::cout << token_a + token_b << '\n';
  return token_a + token_b;
}

int solve(std::vector<TestCase> input) {
